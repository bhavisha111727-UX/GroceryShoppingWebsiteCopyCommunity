...
export function AuthModal({ onClose, onAuthSuccess, initialTab = "login" }: AuthModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const name =
      ...
      "User";
    onAuthSuccess({ name, email: data.user.email! });
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      onAuthSuccess({ name: signupForm.name, email: signupForm.email });
    } else {
        ...
        <div className="bg-primary px-8 pt-10 pb-8 rounded-t-3xl flex-shrink-0">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-white font-bold text-lg tracking-tight">
              The Grocers <span className="font-extrabold">Market</span>
            </span>
          </div>
